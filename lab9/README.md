...
7. Выводы о применении MVC, работе с SQLite, обработке маршрутов и рендеринге шаблонов.
   Мной был создан проект на основе архитектуры MVC: Models (папка models) + view (папка templates) + controllers (папка controllers). Такая архитектура позволяет четко разделить ответственность и вносить изменения в одну сферу проекта, не задевая вторую (например: менять дизайн, не изменяя бизнес-логику). Я использовала SQLite в in-memory режими, что обеспечивает высокую производительность, изоляцию среды (БД обновляется при каждом новом запуске myapp.py) и отсутствие необходимости устанавливать сторонние серверы. Маршрутизация выполнена через анализ пути запроса (urlparse) в методе do_GET. Каждый маршрут (/, /users, /currencies, /user?id=...) обрабатывается отдельным блоком кода, благодаря чему можно легко добавлять новые страницы и явно прослеживать соответствие выполнения нужных страниц по URL. Шаблонизация реализована с помощью Jinja2, использован FileSystemLoader для загрузки шаблонов из папки templates/, шаблоны содержат только разметку и минимальную логику (циклы, условия), что соответствует принципам MVC.
